...
/** 堀田機工製作品（受注時に堀田機工へ注文書を自動送付する対象） */
export function isHottaBracketItem(
  item: Pick<WholesaleCatalogItem, "section">,
): boolean {
  return item.section === "堀田機工";
}

/** 堀田機工品のうち仕切単価が未確定のもの（金額計算から除外し「価格未定」表示） */
export function isHottaPricePendingItem(
  item: Pick<WholesaleCatalogItem, "section" | "wholesalePriceExTax">,
): boolean {
  return isHottaBracketItem(item) && item.wholesalePriceExTax === 0;
}
